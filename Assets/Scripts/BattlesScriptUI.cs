...
public class BattlesScriptUI : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (TheBattleIsOn == true)
        {
            ...
            {
                ValueRounds += 1;
                ValueRoundsUI.text = ValueRounds.ToString();

                GameMinute = 0.0f;
                _battleScript.Battle();

                if (BattleSound.isPlaying == false) BattleSound.Play();

                if ((_battleScript._enemybattleArmyCheck[0] == 0) || (_battleScript._battleArmyCheck[0] == 0))
                {
                    ...
                    else if (_battleScript._enemybattleArmyCheck[0] == 0)
                    {
                        BattleWinnerUI.text = "Победа гномов";
                    }

                    else if (_battleScript._battleArmyCheck[0] == 0)
                    {
